...
#######################################################################################################
    ...
    function Invoke-GetTokens {
        ...
        if ($AccessToken) {
            ...
            Write-Host "[+] Using Managed Ideneity Access Token" -ForegroundColor Gray
            return $global:AccessToken
        }
        ...
        if ($ClientID -and $ClientSecret) {
            ...
            try {
                $resp = Invoke-RestMethod -Method POST -Uri $url -Body $body -ContentType "application/x-www-form-urlencoded"
                $global:AccessToken = $resp.access_token
                Write-Host "[+] Using Service Principal Access Token" -ForegroundColor Gray
                return $global:AccessToken
            }
            catch {
        ...
        while ($true) {
            try {
                ...
                Write-Host "[+] Using Interactive User Access Token  from DeviceCode" -ForegroundColor Gray
                return $global:AccessToken
            }
            catch {
...
#######################################################################################################
    ...
    function Get-Subscriptions {
        ...
        Write-Host "`n[+] Enumeration Subscriptions on ARM ($($a))" -ForegroundColor Gray
        ...
        foreach ($s in $subs) {
            Write-Host "    [^] $($s.SubscriptionName) -> $($s.SubscriptionId) -> $($s.State)" -ForegroundColor DarkGray
        }
        return $subs
    }
...
#######################################################################################################
    ...
    function Get-AllWebApps {
        ...
        $a = TimeNow
        Write-Host "[+] Enumeration of Web Apps ($($a))" -ForegroundColor Gray
        $allApps = @()
        ...
        foreach ($sub in $Subscriptions) {
            ...
            foreach ($app in $resp.value) {
                ...
                if (-not $scmHost -and $detail -and $detail.properties -and $detail.properties.hostNameSslStates) {
                    ...
                    if ($scmEntry) {
                         $scmHost = $scmEntry.name 
                    }
                }
                # Last fallback
                if (-not $scmHost) {
                     $scmHost = "$($app.name).scm.azurewebsites.net" 
                }
                ...
                Write-Host "    [+] $($app.name) : $scmHost" -ForegroundColor DarkGray
        ...
        Write-Host "    [+] Total $($allApps.Count) Web App(s)" -ForegroundColor Gray
        return $allApps
    }
...
###########################################################################################################################################
    ...
    function Check-Permissions {
        ...
        $a = TimeNow
        Write-Host "[+] Enumeration API Permissions on each Web App ($($a))" -ForegroundColor Gray
...
###########################################################################################################################################
    function Set-BasicAuth {
        ...
        $a = TimeNow
        Write-Host " [+] $state Basic Auth ($($a)" -ForegroundColor Gray
...
###########################################################################################################################################
    function Get-PublishingCredentials {
        ...
        $a = TimeNow
        Write-Host "[+] Extracting publishing credentials ($($a)" -ForegroundColor Gray
...
###########################################################################################################################################
    function Invoke-KuduDownload {
        ...
        Write-Host "[$($a)] Downloading: $uri" -ForegroundColor Gray
        Write-Host "[+] Saving to -> $LocalPath" -ForegroundColor Gray
...
###########################################################################################################################################
    function Invoke-KuduUpload {
        ...
        Write-Host "[$($a)] Uploading: $LocalPath ($sizeStr)" -ForegroundColor Cyan
...
###########################################################################################################################################
    function Start-InteractiveShell {
        ...
        Write-Host "[+] SCM Endpoint: $scmHost" -ForegroundColor Gray
...
###################################################################################
    ...
    function main {
        ...
        while ($true) {
            ...
            if ($choice -match "^[Qq]$") {
                Write-Host "`n[+] Bye yafe Sheli" -ForegroundColor Gray
                break
            }
            ...
            Write-Host "`n[+] Enter to: $($selectedApp.Name) [$($selectedApp.ResourceGroup)]" -ForegroundColor Gray
            ...
            if ($wasBasicAuthEnabled) {
                Write-Host "[+] Basic Auth already enabled" -ForegroundColor Gray
            } else {
                Set-BasicAuth -App $selectedApp -Headers $headers -Enable $true
            }
            ...
            Write-Host "`n[+] Disconnecting from $($selectedApp.Name)..." -ForegroundColor Gray
            if (-not $wasBasicAuthEnabled) {
                Set-BasicAuth -App $selectedApp -Headers $headers -Enable $false
            }
            else{
                 Write-Host "[+] Basic Auth was already on" -ForegroundColor Gray
            }
            Write-Host ""
        }
    }
